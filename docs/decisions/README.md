...
| **W3 — data foundation** ✅ **Accepted 2026-08-08** | 0012, 0013, 0014, 0015, 0016, 0017, 0018, 0019, 0023, 0025 | Migration entry conditions: the ADR-0011/0013 multi-schema PoC **PASSED** (9 assertions against a real postgres:16, run 31278155351). The ADR-0014 migration-lint live-fire is **still unmet** — no migration lands until it is. |
...
**Two further entry conditions gate the first migration.** The ADR-0011/0013 multi-schema proof of concept **passed** on 2026-08-08 and is re-runnable via `workflow_dispatch`. The ADR-0014 migration-lint live-fire remains **unmet**: its fixtures must come from the real `drizzle-kit` pipeline, not hand-written SQL, and one deliberately non-compliant migration must fail real CI before the lint is trusted. Neither is advice; both are gates.
