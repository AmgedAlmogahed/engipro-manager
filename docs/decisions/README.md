...
| **W1b — machinery** ✅ **Accepted 2026-08-08** | 0003, 0008, 0037, 0038 | Every other ADR's Enforcement section assumes CI, dependency-cruiser, and the agent guardrails exist. Accepting a decision whose enforcement mechanism is itself unratified is how the predecessor got C1. Ratified **and installed** — running and green, not merely Accepted. **One gap: ADR-0038 is partially installed** — the hook script is landed and tested, but `.claude/settings.json` and the root `CLAUDE.md` are not committed, so the prototype freeze is discipline-only until they are. |
