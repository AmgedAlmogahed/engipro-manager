...
export const Layout: React.FC = () => {
  ...
  return (
    <div className="flex h-screen bg-gray-50 overflow-hidden" dir={direction}>
      {/* Sidebar */}
      <aside className="w-64 bg-slate-900 text-white hidden md:flex flex-col flex-shrink-0 transition-all duration-300 border-e border-slate-800">
        ...
        <nav className="flex-1 overflow-y-auto py-6 px-3 space-y-1">
          <NavItem to="/dashboard" icon={<LayoutDashboard size={20} />} label={language === 'ar' ? 'لوحة التحكم' : 'Dashboard'} />
          <NavItem to="/my-tasks" icon={<CheckSquare size={20} />} label={language === 'ar' ? 'مهامي' : 'My Tasks'} />
          <NavItem to="/projects" icon={<FolderKanban size={20} />} label={language === 'ar' ? 'المشاريع' : 'Projects'} />
          <NavItem to="/authority" icon={<FileText size={20} />} label={language === 'ar' ? 'التراخيص' : 'Authority Tracking'} />
          <NavItem to="/team" icon={<Users size={20} />} label={language === 'ar' ? 'الفريق والموارد' : 'Team & Resources'} />
          <div className="pt-4 mt-4 border-t border-slate-800">
             <p className="px-3 text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">Admin</p>
             <NavItem to="/roles" icon={<ShieldCheck size={20} />} label={language === 'ar' ? 'الأدوار والصلاحيات' : 'Roles & Permissions'} />
          </div>
        </nav>
